 ...
 *
 * 	-支持串口关闭开启
 * 	-接收使用线程查询，定义字节处理函数　void　RxByte_CallBack(std::vector<unsigned char> &data,int fd)　
 * 	-发送支持３种输入类型：unsigned char,vector<unsigned char>,char*
 */
#include "serialport.h"

//接收回调函数
void __attribute__((weak)) RxData_CallBack(std::vector<unsigned char> &data, SerialPort *UART) {
...
bool SerialPort::Open() {
	...
	//开启监听线程
	//pthread_create(&this->listen_thread, NULL, this->RXListen, this);
